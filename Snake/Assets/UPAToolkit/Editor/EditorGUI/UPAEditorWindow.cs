	...
	void OnGUI () {
		...
		if (e.button == 0) {
			...
			if (e.type == EventType.KeyDown) {
				...
				{
					UPADrawer.gotoCenter();
				}
				if (e.keyCode == KeyCode.Alpha8)
				{
					UPADrawer.setGridBGIndex(0);
				}
				if (e.keyCode == KeyCode.Alpha9)
				{
					UPADrawer.setGridBGIndex(1);
				}
				...
				{
					if(EditorUtility.DisplayDialog("Clear all Stored Moves?", "Are you sure you want to clear all previous moves?", "Clear", "Cancel"))
					{
						UPAImage.UPARewindTiles.Clear();
						Debug.Log("Successfully cleared all stored moves");
					}
				}
				//\a
			}
